...
import { OrganiserToggle } from "@/components/founder-toggle";
...
import { nemesisOf, rivalOf } from "@/lib/stats";
...
export default async function MemberPage({
  ...
  // Organisers decide who else organises, including stepping themselves down;
  // only an organiser looking at somebody else's page can mint a way back in.
  const canAdmin = isOrganiser(ctx);
  const canRecover = canAdmin && !isMe;
  const [netC, results, ledgerItems, { open }, split, recoveries, recap] = await Promise.all([
    netOf(tripId, member.id),
    memberResults(tripId, member.id),
    memberLedger(tripId, member.id),
    listMarkets(tripId, member.id),
    memberSplit(tripId, member.id),
    canRecover ? listRecoveries(tripId) : null,
    tripRecap(tripId),
  ]);
  ...
  const nemesis = recap ? nemesisOf(member.id, recap.rivalries) : null;
  const rival = nemesis ? rivalOf(member.id, nemesis) : null;
  const rivalName = rival ? (recap?.memberById.get(rival.id)?.name ?? "someone") : null;
    ...
    <div className="mx-auto max-w-3xl">
          ...
          {rival && rivalName && (
            <p className="mt-1 text-sm">
              ...
              <span className="mono text-xs text-soft">
                {rival.wins}–{rival.losses} head to head
              </span>
            </p>
          )}
        </div>
      ...
      {canAdmin && (
        <section className="mt-7">
          <h2 className="display text-xl font-bold uppercase tracking-wide text-soft">Organiser</h2>
          <div className="mt-3">
            <OrganiserToggle
              tripId={tripId}
              memberId={member.id}
          ...
          <ul className="mt-3 card list">
            {openPositions.map((v) => (
              <li key={v.market.id} className="flex items-center gap-3 px-4 py-2.5 text-sm">
                <Link
                  href={routes.market(tripId, v.market.id)}
          ...
          <ul className="mt-3 card list">
            {results.map((r) => (
              <li key={r.market.id} className="flex items-center gap-3 px-4 py-2.5 text-sm">
                <Link
                  href={routes.market(tripId, r.market.id)}
                  className="min-w-0 flex-1 font-semibold hover:underline"
                >
                  {r.market.question}
                </Link>
